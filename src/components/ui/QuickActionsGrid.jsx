    ...
    {actions.map((action, index) => {
      ...
      return (
        <div
          ...
          onClick={() => alert(`Clicked on "${action.title}"`)} // Placeholder for real functionality
        >
          <Icon size={28} className="text-blue-500 mb-2" />
          <span className="text-gray-900 font-medium text-sm text-center">{action.title}</span>
        </div>
      );
    })}
  </div>
