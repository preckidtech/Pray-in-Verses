        ...
        <div className="space-y-6">
          {content.initiatives.map((initiative, idx) => (
            <div key={idx} className="bg-white p-6 rounded-lg shadow hover:shadow-lg transition-shadow">
              ...
              <p className="mt-2 font-semibold text-[#0C2E8A]">{initiative.impact}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gradient-to-r from-[#0C2E8A] to-[#0C2E8A] text-white py-16 px-8">
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
          {content.goals.map((goal, idx) => (
            <div key={idx} className="bg-white text-gray-800 rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
              <h3 className="text-xl font-bold">{goal.year} - {goal.title}</h3>
              <p className="mt-2">{goal.description}</p>
              <span className="mt-2 inline-block px-3 py-1 bg-[#0C2E8A] text-white rounded-full text-xs">{goal.status}</span>
            </div>
          ))}
        </div>
      </section>
